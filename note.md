...
![img](src/main/resources/img/tcp-four-waveHand.jpg)
...
​	choose 标签按顺序判断其内部 when 标签中的判断条件是否成立，如果有一个成立，则执行相应的 SQL 语句，choose 执行结束；如果都不成立，则执行 otherwise 中的 SQL 语句。这**类似于 switch 语句**，choose 为 switch，when 为 case，otherwise 则为 default。
...
​	PageHelper首先将前端传递的参数保存到page（包含起始页和页大小）这个对象中，接着将page的副本存放入**ThreadLocal中，这样可以保证分页的时候，参数互不影响，接着利用了mybatis提供的**拦截器**，取得ThreadLocal的值，重新**拼装分页SQL**，完成分页。
...
**所有CRUD都有事务支持，单条数据操作基于数据库事务处理，批量操作时业务代码方法上都有事务@Transactional。**
...
​	写缓存，在一些时机再写磁盘，比如定时写，库存不足时写。如果先写缓存，如果宕机的话会丢失数据；可以增加多个redis服务器构成redis集群，降低宕机风险；另外可以通过写日志的方式来减小宕机引起的数据丢失问题。这适用于抢购这一类的短时间的大量读写操作的场景。

​	写数据库，写完后删除缓存，或者写之前就删除缓存。删除缓存再写数据库的话，如果写操作完成之前执行了读操作数据就不一致了；如果阻塞读操作的话，系统吞吐率极度降低。写完数据库再删除缓存，修改前的数据，但保证了数据的最终一致性。这适用于常规的读写的场景。
...
​	**B+树**：关系型数据库最常用的最有效的索引。**InnoDB B+树索引并不能直接找到一个给定键值的具体行，只能找到被查询数据所在的页，然后将数据页读入内存，再在内存中进行查找。**
...
​	InnoDB  B+树**节点（阶）大小**是page，默认情况下，**page的大小为16KB**。为扇区的整数倍。

#### 树高度

​	**B+树的高度一般在2到4层**。也就是说查找某一键值的记录，最多只需要2到4次IO开销。

​	因为非叶子节点存放（key，pointer），key为8个字节（视key的数据类型而定），pointer为8个字节（视机器而定，32位为4B，64位为8B），即非叶子节点能存放16kB/16左右的key，pointer，而叶子节点如果一条数据大小为500B，一个叶子节点大约可存放32条数据。

​	如果高度为3，则可存放数据为：16kB/16 * 16kB/16 * 32，即三千二百万条数据。高度为4的话就可以存放320亿条数据。、

#### 不走索引情况

​	B+树结构决定的不走索引情况：不等于；or；最左匹配，like，没加引号；操作索引列。

1. null判断；
2. !=，<>；
3. **or**；要想使用or，又想让索引生效，or条件中的每个列上必须都有索引；
4. like查询以%开头；
5. **where子句对于索引列  进行算术运算、函数运算**等操作；如 select id from t where num/2=100 ；
6. 列为字符串，而where子句中**没加引号**； 如 SELECT name,age,address FROM user where name = 10
7. 组合索引不满足最左匹配；
8. 如果全表扫描更快，则进行全表扫描而不用索引。
...
​	分支节点存储键值与指针，叶子节点存储键值与记录。



### 辅助索引

​	分支节点存储键值与指针，叶子节点存储键值与主键。



主键索引，索引列唯一，不允许有空值，建立在主键上，是聚簇索引。

唯一索引，索引列唯一，允许有空值，建立在唯一字段上，是辅助索引。

普通索引，建立在普通字段上，是辅助索引。

组合索引，建立在多个字段上，遵循最左前缀集合。
...
​    latch 一般称为闩锁（轻量级的锁），因为其要求锁定的时间必须非常短。若持续的 时间长，则应用的性能会非常差。在InnoDB存储引擎中，latch 又可以分为 mutex (互斥量） 和rwlock (读写锁）。其目的是用来保证并发线程操作临界资源的正确性，并且通常没有死锁检测的机制。

​    **lock的对象是事务，用来锁定的是数据库中的对象**，如表、页、行。并且一般lock的对象仅在事务 commit或rollback后进行释放（不同事务隔离级别释放的时间可能不同）。此外，lock, 正如在大多数数据库中一样，是有死锁机制的。

![image-20210810094305287](.\src\main\resources\img\mysql-latch-lock.jpg)
...
​	InnoDB 存储引擎支持多粒度 (granular) 锁定， 这种锁定允许事务在行级上的 锁和表级上的锁同时存在。为了支待在不同粒度上进行加锁操作， InnoDB 存储引擎支持 一种额外的锁方式， 称之为**意向锁**（Intention Lock） ，意向锁是将锁定的对象分为多个层次。

​	 **意向锁意味着事务希望在更细粒度上进行加锁**。**lnnoDB**存储引擎**意向锁为表级锁**。设计目的主要是为了在一个事务中揭示下一行将被请求的锁类型。其支持两种意向锁：
...
​	**由于lnnoDB支持的是行级锁，因此意向锁其实不会阻塞除全表扫以外的任何请求。**
...
![未命名文件 (7)](.\src\main\resources\img\mysql-lock-procedure.jpg)
...
​	Next-Key Lock : Gap Lock+Record Lock,**下键锁**， 锁定一个范围， 并且锁定记录本身。

​    **Next-Key Lock 是结合了 Gap Lock 和 Record Lock 的一种锁定算法，  InnoDB 对于行的查询都是采用这种锁定算法。例如：索引节点为{10,11,13,20}那么可能被 下键锁 锁住的区间为**

**(-∞, 10]，(10, 11]，(11, 13]，(13, 20]，(20, +∞)。左开右闭区间，对应于B+树子节点值小于等于索引节点的键值。**
...
![image-20210810105408740](.\src\main\resources\img\mysql-cmd-1.jpg)

### 测试读写冲突

![image-20210810111906051](.\src\main\resources\img\mysql-cmd-2.jpg)

1. for update指定加写锁，语句以分号结束，开启事务使用begin，结束使用commit；
2. 加写锁不影响读，因为版本快照；
...
![image-20210810112146276](.\src\main\resources\img\mysql-cmd-3.jpg)

1. 写写操作可能会发生冲突，即事务2想要在事务1已加锁的索引上加锁。
...
![image-20210810121313010](.\src\main\resources\img\mysql-cmd-4.jpg)



#### 写写操作可能不发生冲突

![image-20210810135204705](.\src\main\resources\img\mysql-cmd-5.jpg)
...
​	微服务不会直接从一个微服务向另一个微服务发出post/put API调用,而是会向kafka生成一条消息,然后由一个微服务消费。

​	Pub/Sub 允许**服务异步通信**，延迟时间大约为 100 毫秒。
...
​	消息系统按照消息传递模式可分为**点对点模式和发布-订阅模式**（Kafka）。
...
​	此模式下**生产者发送一条消息，只有一个消费者能够接收到（1对1）**。
...
​	容器：容器只是将应用程序和它们所需的依赖打包，直接将应用程序运行在宿主计算机的内核上。说白了就是将依赖与APP一起打包。
...
![](src/main/resources/img/docker-commands.jpg)
...
​	**动静态资源分离**——运用Nginx的反向代理功能分发请求：所有**动态资源的请求交给Tomcat**，而**静态资源的请求由Nginx返回到浏览器**（例如图片、视频、CSS、JavaScript文件等），这样能大大减轻Tomcat的压力。

​	负载均衡，当业务压力增大时，可能一个Tomcat的实例不足以处理，那么这时可以启动多个Tomcat实例进行水平扩展，而Nginx的负载均衡功能可以把请求通过算法分发到各个不同的实例进行处理。

​	Apache/Nginx 应该叫做 HTTP Server；**HTTP Server 关心的是 HTTP 协议层面的传输和访问控制**。

​	而 Tomcat 则是一个 Application Server ，或者更准确的来说，是一个「Servlet/JSP」应用的容器（Ruby/Python 等其他语言开发的应用也无法直接运行在 Tomcat 上）。**Application Server是一个应用执行的容器。支持开发语言的 Runtime（对于 Tomcat 来说，就是 JRE），保证应用能够在应用服务器上正常运行。**
...
​		个人理解：秒杀系统还是商品买卖问题，主要还是涉及到三张表——用户表，订单表，商品表。
...
​	用户表只保存已经报名参加活动的用户。可以放在redis等缓存里面，减少查库时间。

​	商品表的话也是将促销商品单独拿出来作为一张表。

​	订单表也是如此。避免写的时候与其他购买请求相互阻塞。
...
​	高并发下会在活动开始时，服务器接收到的请求达到一个峰值。那如何减小峰值呢。

​	1. 从用户考虑的话，延长用户请求时间可以在一定程度上降低峰值。即让用户答题、输入验证码等。

​	2. 从服务器角度考虑的话，可以将所有请求放到消息队列中，然后后台服务器消费这些消息。
...
1. 可以将库存 存放在redis中，但这只适用于上述的第二种方案，因为redis回滚机制。

2. 数据库里面库存判断，即基于数据库的事务回滚机制。
...
​	如何保证用户点击“抢购”后立马得到抢购成功与否的结果呢。请求添加到mq里面就返回请求成功之类的消息。
...
![img](src/main/resources/img/spike-base.jpg)
...
​	秒杀系统设计的第一个原则就是将热点数据隔离出来，不要让1%的请求影响到另外99%的请求。
...
​	秒杀系统所使用的数据大部分都是热数据。使用单独的redis等数据库集群来存储热点数据。



## 动静分离

​	把整个页面缓存在用户浏览器，实际请求的有效数据只是一个“抢购”按钮。即ajax异步请求。



## 基于时间片削峰

​	抢购时，增加秒杀答题，这样既可以防止秒杀器，也可以将峰值时间拉长。消息队列削峰。

​	前端对答题结果等信息进行判断，减少无效请求。



数据流向

![img](src/main/resources/img/spike-base1.jpg)
...
​	提前发现不了突然成为热点的商品成为热点数据，这种就要通过实时热点数据分析。
...
​	leader负责系统设计，比如数据库，需求文档等。

​	同事A负责学习google API及其使用，例如企业注册两种方式选取更简洁的一种。

​	同事B负责系统框架搭建和系统协作实现，诸如用户管理，日志管理、策略管理等。

​	我负责实现新的功能，在同事B搭的架子的基础上，使用同事A整理的API，实现leader制定的部分需求；比如用户密码的安全传输与存储、企业信息维护（与google同步）、定时获取设备信息、策略生成等。





问题1.  架子上使用google API时出现jar包版本不一致问题；

问题2.  事务处理，逻辑应该是先修改本地数据库，再去修改谷歌端；Google端修改失败时是抛出的 IO异常，因此事务需要加上rollbackFor = Exception.class；
